...
const FunderManagement = () => {
  ...
  //Listing
  const allFunderListing = () => {
    const data = { user_type: "FU", is_approved: "approved", };
  ...
  // Searching
  const searchAllFunders = (value) => {
    const data = { search_data: value.trim(), modelname: "user", app_name: "baseapp", };
    dispatch(searchData(data))
      .then((res) => {
        let newArray = res?.data?.filter((filterSearch) => filterSearch?.user_type === "FU");
        setInitialUser(newArray);
      })
      .catch((err) => allFunderListing());
  };
  ...
  const handleVerifyOk = () => {
    ...
    dispatch(verifyUser(verify))
      .then((response) => {
        form.setFieldsValue({ funderSearch: "" });
        toast.success(response?.message);
        allFunderListing();
      ...
      <div className="patients-search">
        <div className="custom-search">
          ...
          <DebounceInput
            ...
            onChange={(e) => searchAllFunders(e.target.value)}
          />
        </div>
        <div className="btn-list">
          <Button
            type="primary"
            htmlType="submit"
            onClick={() => navigate("/new-funder-requests")}
          >
            New Funder Requests
          </Button>
        </div>
