...
const BlockedBrokers = () => {
  ...
  useEffect(() => {
    blockedBrokerListing()
  }, [])


  //Listing
  const blockedBrokerListing = () => {
    const data = { user_type: "BR", }
  ...
  // Searching
  const searchBlockedBroker = (value) => {
    const data = { query: value.trim(), user_type: 'BR' }
    dispatch(searchBlockedUser(data)).then(res => setInitialUser(res.data)).catch(err => blockedBrokerListing())
  }
      ...
      <div className='patients-search'>
        <div className='custom-search'>
          ...
          <DebounceInput
            ...
            onChange={(e) => searchBlockedBroker(e.target.value)}
          />
        </div>
      </div>
