...
import { deletePost, getBrokerPostList } from '../../actions/Post';
...
const BrokerPosts = () => {
  ...
  //Listing
  const viewBrokerPostList = () => {
    dispatch(getBrokerPostList())
  ...
  // Searching
  const searchBrokerPosts = (value) => {
    const data = { search_data: value.trim(), modelname: "feed", app_name: 'allpost', news_feed_type: 'BR' }
    dispatch(searchData(data)).then(res => setPost(res.data)).catch(err => viewBrokerPostList())
  }
      ...
      <div className='patients-search'>
        <div className='custom-search'>
          ...
          <DebounceInput
            ...
            onChange={(e) => searchBrokerPosts(e.target.value)}
          />
        </div>
      </div>
        ...
        <div className='post-detail'>
          ...
          <div className='footer-post'>
            ...
            {postId?.comment === 0 ?
              ...
              :
              <span className='comment' onClick={() => navigate(`/post-management/all-posts/${postId?.id}/broker-post-comments`)} ><CommentIcon />{postId?.comment}  {postId?.comment <= 1 ? "Comment" : "Comments"}</span>
            }
          </div>
        </div>
      </Modal>
